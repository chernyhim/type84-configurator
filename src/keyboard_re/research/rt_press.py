"""
Research runner for Rapid Trigger (RT) Press encoding on IO Type 84 Magnetic Black.

Executes a series of controlled RT Press modifications on a single key in Hall Profile 1:
...
- Detects exact changed byte offsets
- Verifies that Actuation, RT Release, Flags, and all other 83 keys remain 100% untouched
...
def run_rt_press_research(
    key_name: str = "A",
    test_values: Optional[List[float]] = None,
    output_dir: str = "captures/research/rt_press",
    auto_confirm: bool = False,
    quiet: bool = False,
) -> Tuple[bool, List[RtPressExperimentStep]]:
    """
    Run controlled empirical RT Press discovery on a single physical key.
    """
    ...
    steps: List[RtPressExperimentStep] = []
    for val in test_values:
        desc = "OFF (Rapid Trigger disabled)" if val == 0.0 else f"RT Press = {val:.2f} mm"
        steps.append(RtPressExperimentStep(target_mm=val, description=desc))
    ...
    try:
        ...
        print(f"RT Press Addr:  0x{rt_press_addr:04X}..0x{rt_press_addr+1:04X} (uint16_le)")
        ...
        for idx, step in enumerate(steps):
            ...
            current_press_raw = cfg.rt_press
            current_release_mm = cfg.rt_release_mm
            current_flags = cfg.flags

            target_raw = step.raw_uint16
            old_bytes = struct.pack("<H", current_press_raw)
            new_bytes = struct.pack("<H", target_raw)

            print(f"Keyboard:          {transport.actual_product_name}")
            print(f"Key:               {resolved_key}")
            print(f"Current RT Press:  {current_press_mm:.2f} mm (raw: 0x{current_press_raw:04X}, dec: {current_press_raw})")
            print(f"Target RT Press:   {step.target_mm:.2f} mm (raw: 0x{target_raw:04X}, dec: {target_raw})")
            print(f"Changed byte(s):   0x{rt_press_addr:04X}..0x{rt_press_addr+1:04X}: {old_bytes.hex(' ').upper()} -> {new_bytes.hex(' ').upper()}")
            print(f"Preserved fields:  Actuation={current_actuation:.2f}mm, RT_Rel={current_release_mm:.2f}mm, Flags=0x{current_flags:04X}")
            print("=" * 64)
            ...
            # Only modify rt_press! Leave actuation, rt_release, flags untouched.
            cfg.rt_press = target_raw
            ...
            expected_offsets = {rt_press_addr, rt_press_addr + 1}
            ...
            step.read_back_mm = rb_cfg.rt_press_mm
            step.read_back_uint16 = rb_cfg.rt_press
            ...
            step.success = True
            print(f"STEP {idx+1} SUCCESS: RT Press = {step.read_back_mm:.2f} mm (raw uint16: 0x{step.read_back_uint16:04X})")
...
def print_summary_table(steps: List[RtPressExperimentStep]) -> None:
    """
    Format and display research table: Value (mm) -> Raw uint16.
    """
    print("\n" + "=" * 78)
    print("RT PRESS ENCODING EMPIRICAL TABLE (IO Type 84 Magnetic Black)")
    print("=" * 78)
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(
        description="Empirical research runner for Rapid Trigger (RT) Press encoding."
    )
    parser.add_argument("--key", default="A", help="Physical key name to test (default: A)")
    parser.add_argument("--values", nargs="+", type=float, default=[0.00, 0.10, 0.20, 0.50, 1.00, 0.00], help="List of RT Press values in mm to test sequentially")
    ...
    parser.add_argument("-o", "--output-dir", default="captures/research/rt_press", help="Directory to save before/after images")

    args = parser.parse_args(argv)
    success, steps = run_rt_press_research(
        key_name=args.key,
