...
print("=== TABLE 55 12 (Remap Layer 1) ===")
for bank in range(8):
    print(f"\n--- BANK {bank} (slots {bank*16}..{bank*16+15}) ---")
    for col in range(16):
        idx = bank * 16 + col
        offset = idx * 4
        ...
        print(f"  Col {col:2d} (Slot {idx:3d}, Off {offset:3d}): {rec.hex(' ')} | b0={b0:02x} b1={b1:02x} b2={b2:02x} b3={b3:02x} -> {name}")
